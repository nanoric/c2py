...
class Cursor(Structure):
    ...
    def type(self):
        ...
        if not hasattr(self, '_type'):
            self._type = conf.lib.clang_getCursorType(self)  # type: Type
    ...
    def result_type(self):
        ...
        if not hasattr(self, '_result_type'):
            self._result_type = conf.lib.clang_getCursorResultType(self)  # type: Type
    ...
    @property
    def underlying_typedef_type(self):  # type: ()->Type
        ...
        if not hasattr(self, '_underlying_type'):
            ...
            self._underlying_type = conf.lib.clang_getTypedefDeclUnderlyingType(self) # type: Type
    ...
    @property
    def semantic_parent(self):  # type: ()->Cursor
        """Return the semantic parent for this cursor."""
        if not hasattr(self, '_semantic_parent'):
            self._semantic_parent = conf.lib.clang_getCursorSemanticParent(self) # type: Cursor
    ...
    def referenced(self):
        ...
        if not hasattr(self, '_referenced'):
            self._referenced = conf.lib.clang_getCursorReferenced(self)  # type: Cursor
...
class TranslationUnit(ClangObject):
    ...
    # Cache code completion information on process. This adds time to parsing but
    # speeds up code completion.
    PARSE_CACHE_COMPLETION_RESULTS = 8

    # Flags with values 16 and 32 are deprecated and intentionally omitted.

    # Do not process function bodies. This is useful if you only care about
    ...
    def from_source(cls, filename, args=None, unsaved_files=None, options=0,
        ...
        To process source from the filesystem, the filename of the file to process
        is specified by the filename argument. Or, filename could be None and
        the args list would contain the filename(s) to process.

        To process source from an in-memory buffer, set filename to the virtual
